...
$_SESSION['idpedido'];
 ...
 $idcliente = mysqli_real_escape_string($mysqli, $_SESSION['cliente']);
$idpedido =  mysqli_real_escape_string($mysqli, $_SESSION['idpedido']);
        ...
        $sql = "SELECT apellidocli, nombrecliente from tblcliente where id=' $idcliente '";
        $resultado2 = $mysqli -> query($sql);
        while ($fila = $resultado2 -> fetch_assoc()){
          $nombre=$fila['nombrecli'];
          $apellido= $fila['apellidocli'];
}
        ...
        <div style="margin: 10px;"></p>Cliente: '.$nombre.' '.$apellido.'</p></div></div>
      ';
